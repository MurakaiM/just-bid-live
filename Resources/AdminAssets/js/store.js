...
function onLoad(){
  ...
  tabs = new Tabs({
    ...
    },
    {
      ...
      home : {
        onOpen : (head,segment) =>{ dataStats.load(head,segment)  },
        onClose : () => {   }
      }
  });
...
function SetUpDisabled(){
  ...
  const renewModal = new Modal({
    ...
    middleware : body => {
      ...
      button.click( e => {
        button.addClass('loading');
        POST('/seller/product/renew', { uid : currentItem.arr[currentItem.i].prUid })
          .then( data => {
            currentItem.arr.splice(currentItem.i, 1);
          ...
          .catch( error => { renewModal.toggleState(); button.removeClass('loading'); });
      });
    }
  }, true);
  ...
  const finishModal = new Modal({
    ...
    middleware : body => {
      ...
      button.click( e => {
        button.addClass('loading');
        POST('/seller/product/remove', { uid : currentItem.arr[currentItem.i].prUid })
          .then( data => {
            currentItem.arr.splice(currentItem.i, 1);
            tableDisabled.redraw();
            button.removeClass('loading');
